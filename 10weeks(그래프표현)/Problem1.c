...
void insertIncidence(int v1, int v2, INCIDENCE* incidence, GRAPH* graph) {
	...
	if (p == NULL)
		vertices[v1].incidence = incidence; //첫 노드

	else { //오름차순 삽입
		//head와 비교
		if (edge[p->edge].vertex1 == v1)
			other = edge[p->edge].vertex2;
		else
			other = edge[p->edge].vertex1;
		...
		if (other > v2) { //head에서 교환
			...
			while (p->next != NULL) {
				if (edge[p->next->edge].vertex1 == v1)
					other = edge[p->next->edge].vertex2;
				else
					other = edge[p->next->edge].vertex1;
...
void updateEdge(int v1, int v2, int w, GRAPH* graph) {
	...
	if (p == NULL)
		...
		if (check == 0)
			insertEdge(v1, v2, w, graph); //새로운 간선 생성
	}
}
