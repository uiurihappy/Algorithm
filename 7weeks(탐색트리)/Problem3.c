...
void searchAndFixAfterRemoval(NODE** node, NODE* w);
...
void searchAndFixAfterRemoval(NODE** node, NODE* w) {
    ...
    //개조
    NODE* b = restructure(node, x, y, z);

    if (b == *node) 
        return;
    
    //균형을 맞추기 위한 재귀
    searchAndFixAfterRemoval(node, b->parent);
...
int removeElement(NODE** node, int k) {
    ...
    searchAndFixAfterRemoval(node, zs->parent);
